...
def create_vmwarecs(cmd, client,
    ...
    return client.create_or_update(resource_group_name=resource_group, referer=referer, dedicated_cloud_node_name=dedicated_cloud_node_name, dedicated_cloud_node_request=body)
...
def update_vmwarecs(cmd, client, body,
                    ...
                    sku_tier=None,
                    tags=None):
    body = client.get(resource_group_name=resource_group, dedicated_cloud_node_name=dedicated_cloud_node_name).as_dict()
    body.location = location  # str
    body.availability_zone_id = availability_zone_id  # str
    body.nodes_count = nodes_count  # number
    body.placement_group_id = placement_group_id  # str
...
def create_vmwarecs(cmd, client,
    ...
    body['tags'] = tags  # dictionary
    return client.create_or_update(resource_group_name=resource_group, dedicated_cloud_service_name=dedicated_cloud_service_name, dedicated_cloud_service_request=body)
...
def update_vmwarecs(cmd, client, body,
                    ...
                    service_url=None,
                    tags=None):
    body = client.get(resource_group_name=resource_group, dedicated_cloud_service_name=dedicated_cloud_service_name).as_dict()
    body.location = location  # str
    body.gateway_subnet = gateway_subnet  # str
    body.nodes = nodes  # number
    body.service_url = service_url  # str
...
def create_vmwarecs(cmd, client,
    ...
    body['tags'] = tags  # dictionary
    return client.create_or_update(resource_group_name=resource_group, referer=referer, virtual_machine_name=virtual_machine_name, virtual_machine_request=body)
...
def update_vmwarecs(cmd, client, body,
                    ...
                    v_sphere_networks=None,
                    tags=None):
    body = client.get(resource_group_name=resource_group, virtual_machine_name=virtual_machine_name).as_dict()
    body.location = location  # str
    body.amount_of_ram = amount_of_ram  # number
    body.disks = json.loads(disks) if isinstance(disks, str) else disks
    body.expose_to_guest_vm = expose_to_guest_vm  # boolean
