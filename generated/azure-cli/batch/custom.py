...
def create_batch(cmd, client,
    ...
    body['key_vault_reference'] = json.loads(key_vault_reference) if isinstance(key_vault_reference, str) else key_vault_reference
...
def create_batch_pool(cmd, client,
    ...
    body['deployment_configuration'] = json.loads(deployment_configuration) if isinstance(deployment_configuration, str) else deployment_configuration
    body['scale_settings'] = json.loads(scale_settings) if isinstance(scale_settings, str) else scale_settings
    body['inter_node_communication'] = inter_node_communication # str
    body['network_configuration'] = json.loads(network_configuration) if isinstance(network_configuration, str) else network_configuration
    body['max_tasks_per_node'] = max_tasks_per_node # number
    body['task_scheduling_policy'] = json.loads(task_scheduling_policy) if isinstance(task_scheduling_policy, str) else task_scheduling_policy
    body['user_accounts'] = json.loads(user_accounts) if isinstance(user_accounts, str) else user_accounts
    body['metadata'] = json.loads(metadata) if isinstance(metadata, str) else metadata
    body['start_task'] = json.loads(start_task) if isinstance(start_task, str) else start_task
    body['certificates'] = json.loads(certificates) if isinstance(certificates, str) else certificates
    body['application_packages'] = json.loads(application_packages) if isinstance(application_packages, str) else application_packages
    body['application_licenses'] = json.loads(application_licenses) if isinstance(application_licenses, str) else application_licenses
