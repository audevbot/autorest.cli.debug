...
class AzureRMReplicationRecoveryServicesProviders(AzureRMModuleBaseExt):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.replication_recovery_services_providers.create(resource_name=self.resource_name,
                                                                                           resource_group_name=self.resource_group,
                                                                                           fabric_name=self.fabric_name,
                                                                                           provider_name=self.name,
                                                                                           add_provider_input=self.addProviderInput)
            else:
                response = self.mgmt_client.replication_recovery_services_providers.update()
    ...
    def delete_resource(self):
        ...
        try:
            response = self.mgmt_client.replication_recovery_services_providers.delete(resource_name=self.resource_name,
                                                                                       resource_group_name=self.resource_group,
                                                                                       fabric_name=self.fabric_name,
                                                                                       provider_name=self.name)
    ...
    def get_resource(self):
        ...
        found = False
        try:
            response = self.mgmt_client.replication_recovery_services_providers.get(resource_name=self.resource_name,
                                                                                    resource_group_name=self.resource_group,
                                                                                    fabric_name=self.fabric_name,
                                                                                    provider_name=self.name)
        except CloudError as e:
