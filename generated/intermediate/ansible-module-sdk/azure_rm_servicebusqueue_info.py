...
class AzureRMQueuesInfo(AzureRMModuleBase):
    ...
    def getauthorizationrule(self):
        ...
        try:
            response = self.mgmt_client.queues.get_authorization_rule(resource_group_name=self.resource_group,
    ...
    def listauthorizationrules(self):
        ...
        try:
            response = self.mgmt_client.queues.list_authorization_rules(resource_group_name=self.resource_group,
                                                                        namespace_name=self.namespace_name,
                                                                        queue_name=self.queue_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
