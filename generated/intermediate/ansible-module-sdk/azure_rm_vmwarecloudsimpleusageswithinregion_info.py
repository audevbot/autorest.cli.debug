...
DOCUMENTATION = '''
---
module: azure_rm_vmwarecloudsimpleusageswithinregion_info
version_added: '2.9'
short_description: Get UsagesWithinRegion info.
description:
  - Get info of UsagesWithinRegion.
options:
  region_id:
    description:
...
RETURN = '''
usages_within_region:
  description: >-
    A list of dict results where the key is the name of the UsagesWithinRegion
    and the values are the facts for that UsagesWithinRegion.
  returned: always
  type: complex
  contains:
    usageswithinregion_name:
...
class AzureRMUsagesWithinRegionInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMUsagesWithinRegionInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.region_id is not None):
            self.results['usages_within_region'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.usages_within_region.list(region_id=self.region_id)
