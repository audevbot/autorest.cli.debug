...
class AzureRMReplicationProtectionContainersInfo(AzureRMModuleBase):
    ...
    def listbyreplicationfabrics(self):
        ...
        try:
            response = self.mgmt_client.replication_protection_containers.list_by_replication_fabrics(resource_name=self.resource_name,
                                                                                                      resource_group_name=self.resource_group,
                                                                                                      fabric_name=self.fabric_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.replication_protection_containers.list(resource_name=self.resource_name,
                                                                               resource_group_name=self.resource_group)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
