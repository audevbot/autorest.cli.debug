...
DOCUMENTATION = '''
---
module: automationjobstream_info
version_added: '2.9'
...
- name: List job streams by job name
  azure.rm.automationjobstream.info:
...
class AzureRMJobStreamInfo(AzureRMModuleBase):
    ...
    def get(self):
        ...
        self.url = self.url.replace('{{ job_name }}', self.job_name)
        self.url = self.url.replace('{{ stream_name }}', self.name)
